...
file mkdir $synthDir
file delete -force $synthDir/${pm3_v1}
file mkdir $synthDir/${pm3_v1}
puts "#HD: Running synthesis for block ${pm3_v1}"
create_project -in_memory -part ${device}
set_property source_mgmt_mode All [current_project]
set_property board_part ${board} [current_project]
add_files $rtlDir/${pm3_v1}/${pm3_v1}.v
synth_design -mode out_of_context -flatten_hierarchy rebuilt -top ${pm3} -part ${device}
write_checkpoint -force $synthDir/${pm3_v1}/${pm3}_synth.dcp
report_utilization -file $synthDir/${pm3_v1}/${pm3}_utilization_synth.rpt
close_project
puts "#HD: Synthesis of module ${pm3_v1} complete\n"

#建立user4可重构模块综合目录,综合user4可重构模块
file delete -force $synthDir/${pm4_v1}
file mkdir $synthDir/${pm4_v1}
puts "#HD: Running synthesis for block ${pm4_v1}"
create_project -in_memory -part ${device}
set_property source_mgmt_mode All [current_project]
set_property board_part ${board} [current_project]
add_files $rtlDir/${pm4_v1}/${pm4_v1}.v
synth_design -mode out_of_context -flatten_hierarchy rebuilt -top ${pm4} -part ${device}
write_checkpoint -force $synthDir/${pm4_v1}/${pm4}_synth.dcp 
report_utilization -file $synthDir/${pm4_v1}/${pm4}_utilization_synth.rpt
close_project
...
add_file $synthDir/${pm4_v1}/${pm4}_synth.dcp
set_property SCOPED_TO_CELLS { inst_alu_user4 } [get_files $synthDir/${pm4_v1}/${pm4}_synth.dcp]
add_file $synthDir/${pm3_v1}/${pm3}_synth.dcp
set_property SCOPED_TO_CELLS { inst_alu_user3 } [get_files $synthDir/${pm3_v1}/${pm3}_synth.dcp]
add_file $synthDir/${pm2_v1}/${pm2}_synth.dcp
set_property SCOPED_TO_CELLS { inst_alu_user2 } [get_files $synthDir/${pm2_v1}/${pm2}_synth.dcp]
add_file $synthDir/${pm1_v1}/${pm1}_synth.dcp
set_property SCOPED_TO_CELLS { inst_alu_user1 } [get_files $synthDir/${pm1_v1}/${pm1}_synth.dcp]
